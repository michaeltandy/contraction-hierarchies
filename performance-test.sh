...
# aws ec2 request-spot-instances --dry-run --spot-price 0.04 --launch-specification "{\"ImageId\":\"ami-7842030f\",\"InstanceType\":\"r3.large\",\"KeyName\":\"mtandy home i7 pc\",\"IamInstanceProfile\":{\"Name\":\"s3access\"},\"SecurityGroupIds\":[\"sg-2e039e59\"],\"BlockDeviceMappings\":[{\"VirtualName\":\"ephemeral0\",\"DeviceName\":\"/dev/sdb\"}],\"UserData\":\"\"}"
...
for i in `seq 1 10`;
do
    java -cp /mnt/ch/contraction-hierarchies/target/ch-1.0-SNAPSHOT.jar -Xmx12g -Xms8g uk.me.mjt.ch.BenchmarkUk | tee LoadAndPathUk-$i.txt
    aws --region=us-west-1 s3 cp LoadAndPathUk-$i.txt s3://ch-test-mjt/$instance_type/$instance_id/
done
...
aws --region=us-west-1 s3 cp /mnt/runtimes.txt s3://ch-test-mjt/$instance_type/$instance_id/
